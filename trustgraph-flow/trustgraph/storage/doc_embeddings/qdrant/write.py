...
class Processor(CollectionConfigHandler, DocumentEmbeddingsStoreService):
    ...
    async def store_document_embeddings(self, message):
        ...
        for emb in message.chunks:
            ...
            # ===== DEFENSIVE FIX: Handle None and empty vectors =====
            # Skip chunks with None vectors (occurs when chunks exceed embedding model context limit)
            if emb.vectors is None:
                logger.warning(
                    f"Chunk has no vectors (vectors=None), skipping. "
                    f"This typically occurs when the chunk exceeds the embedding model's context window. "
